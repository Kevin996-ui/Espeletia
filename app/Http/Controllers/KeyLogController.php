...

{
...

    {
        ...
        $keyTypes = KeyType::whereNotIn('name', $usedKeyCodes)->get(['name', 'area']);
...

    {
        ...
        $keyInUse = KeyLog::where('key_code', $request->key_code)

                          ->whereNull('key_returned_at')

                          ->exists();

        if ($keyInUse) {

            return back()->withErrors(['key_code' => 'Esta llave ya está en uso y no puede ser asignada.'])->withInput();
        ...
        $keyType = KeyType::where('name', $request->key_code)->first();
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($keyLogs)
                ...
                ->addColumn('action', function ($keyLog) {
                    ...
                    return '
<a href="' . route('keylog.edit', $keyLog->id) . '" class="btn btn-warning btn-sm">Editar</a>
...
<button type="submit" class="btn btn-danger btn-sm">Eliminar</button>
...

    {
        ...
        if ($format === 'csv') {
            ...
            $filename = 'reporte_llaves_' . now()->format('Ymd_His') . '.csv';

            return Excel::download(new KeyLogExport($keyLogs), $filename);

        }

        if ($format === 'pdf') {

            $pdf = Pdf::loadView('keylog_export', ['keyLogs' => $keyLogs])->setPaper('a4', 'landscape');
